...
FW.results <- function(X,BB){
  ...
  Xt3 <- X[12]
  Xt4 <- X[13]
  results <- c(L = sum(Bplot[X>0]>0),
               Ld = (sum(Bplot[X>0]>0))/(sum(X>0)),
               C  = sum(Bplot[X>0]>0)/sum(X>0)^2,
               nT = sum(any(Xt1>0), any(Xt2>0), any(Xt3>0), any(Xt4>0)) )
  return(results)
}
