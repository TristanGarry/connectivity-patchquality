...
df.pca <- df.pca %>% # find centroids
  ...
  mutate(centr_PC1 = mean(PC1), centr_PC2 = mean(PC2))
df.pca <- df.pca %>% # find distance
  mutate(dist = sqrt((PC1 - centr_PC1)^2 + (PC2 - centr_PC2)^2)) ## METHOD 1
#df.pca <- df.pca[df.pca$time == 500,]
df.pca$test1 <- rep(df.pca[df.pca$connectivity==1 & df.pca$quality==0,]$centr_PC1, times = 24)
df.pca$test2 <- rep(df.pca[df.pca$connectivity==1 & df.pca$quality==0,]$centr_PC2, times = 24)
df.pca <- df.pca %>% # find distance
  mutate(dist2 = sqrt((PC1 - test1)^2 + (PC2 - test2)^2)) ## METHOD 2
...
for (conn in 2:4){
  pdf(paste0("rawplots/effsize/",connec[conn] ,"test.pdf"))
