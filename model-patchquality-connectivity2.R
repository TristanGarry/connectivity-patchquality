...
# growth rates
r.source <- c(0,0,0,0,  0, 0.92, 0,  0, 0,  0,  0, 1.78, 0)
r.sink   <- c(0,0,0,0,  0, 0.16, 0,  0, 0,  0,  0, 2.04, 0)
...
# food web
FW <- read.csv(file="foodweb2.csv", header=T)
# number of species
nSp <- nrow(FW)
  ...
  for(ss in ss.prop){
    repeat{
      for(r in 1:reps){
        ...
        Env_Opt <- matrix(0.5, nSp, nEnv)
        # environmental condition in each patch per species
        Env <- matrix(rep(1.0), nrow=nSp, ncol=numCom)
        # environmental match
...
#          Xstoch <- matrix (1, nSp, numCom) ## 20% of cells affected  
 #         stc <- 1
  #        for(stc in 1:(0.2*length(Xstoch))){ 
   #         i <- as.integer(sample(1:nSp, 1))
    #        j <- as.integer(sample(1:numCom, 1))
     #       Xstoch[i,j] <- runif(1, min=0.5, max=1)
      #      stc <- stc + 1
       #   } 
    ...
    #          Xstoch[i,j] <- runif(1, min = 0.8, max = 1.2)
     #       }
      #    }
          ...
          Xt <- X * Xstoch + interactions + Immigrants - Migrants
        ...
        results$div_r [(((loop_id-1)*5)+1):(loop_id*5)] <- vegan::diversity((apply(Xd[,-1:-4], 2, sum)),
                                                                            index="simpson")
        # beta diversity
        results$div_b [(((loop_id-1)*5)+1):(loop_id*5)] <- vegan::diversity((apply(Xd[,-1:-4], 2, sum)), 
                                                                            index="simpson") -
          mean(vegan::diversity(Xd[,-1:-4], index="simpson"))
        # richness
        ...
        # evenness
        results$eve_l [(((loop_id-1)*5)+1):(loop_id*5)] <- vegan::diversity(Xd[,-1:-4], index="simpson") /
          log(specnumber(Xd[,-1:-4])) 
        results$eve_r [(((loop_id-1)*5)+1):(loop_id*5)] <- vegan::diversity((apply(Xd[,-1:-4], 2, sum)), 
                                                                            index="simpson") /
          specnumber((apply(Xd[,-1:-4], 2, sum))) 
        # productivity
